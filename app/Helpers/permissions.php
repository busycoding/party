...
//dd($className); 
            if ($className == 'company' && in_array($method, ['edit', 'update', 'destroy', 'restore', 'forceDestroy']))
            {
                $id = !is_null($id) ? $id : $request->route('party');

                // if the current user has not update-others-company/delete-others-company permission
                // make sure she/he only modify his/her own company
                if ( $id &&
                    (!$currentUser->can('update-others-company') || !$currentUser->can('delete-others-company')) )
                {
                    $company = \App\Company::withTrashed()->find($id);
                    // Cannot edit someone elses company
